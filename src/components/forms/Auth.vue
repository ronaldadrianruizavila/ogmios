    ...
    <div>
        <v-form class="mt-4" id="nativeForm" v-model="valid">
            ...
            <v-text-field
                    ...
                    required
            ></v-text-field>
            <v-btn class="mt-4" depressed :block="$vuetify.breakpoint.smAndDown" round @click="submit" color="primary" dark :disable="!valid">
                {{$t(`${action}.submit`)}}
            </v-btn>
        </v-form>
    </div>
    ...
    export default {
        ...
        data: () => ({
            ...
            passwordConfirmationRules: [
                (v) => !!v || this.$t('validations.required', {field: 'Password'}),
                (v) => v.length >= 6 || this.$t('validations.minLength', {field: 'Password', length: 6}),
                (v) => v === this.password || this.$t('validations.minLength', {field: 'Password', length: 6})
            ]
        }),
        methods: {
            submit() {
